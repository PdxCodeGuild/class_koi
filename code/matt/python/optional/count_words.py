...
words.sort(key=lambda tup: tup[1], reverse=True)
# begin displaying results
print(f'\nThe top {len(words) if len(words) < 10 else "10"} most used words are:\n\nCOUNT\tWORD\n{"="*20}')
...
for i, word in enumerate(words_list):
    if i < len(words_list) - 1:
        # count first instance of word as 1, increment existing instances
        if words_list[i + 1] in word_dict[word]:
            word_dict[word][words_list[i + 1]] += 1
        else:
            word_dict[word][words_list[i + 1]] = 1

# prompt for user to select word to search by
search_word = valid_str('Please enter a word to search for: ',all_words,None,False)

# create list of tuples from nested dictionary with word and count
words = list(word_dict[search_word].items())
# sort largest to smallest, based on count
words.sort(key=lambda tup: tup[1], reverse=True)
# begin displaying results
print(f'\nThe top {len(words) if len(words) < 10 else "10"} most used words after {search_word} are:\n\nCOUNT\tWORD\n{"="*20}')
# print the top 10 words, or all words if there are fewer than 10
for i in range(min(10, len(words))):
    print(f'{words[i][1]}\t{words[i][0]}')