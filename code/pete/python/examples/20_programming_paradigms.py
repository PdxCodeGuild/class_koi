...
def check_withdrawal(state, amount):
...
def withdraw(state, amount):
    if check_withdrawal(state, amount):
        state['balance'] -= amount
...
# books = Book.objects.all().order_by('-author').filter(available=True)
#         # all the book objects # order by author # if available
