...
translator = str.maketrans('','', string.punctuation) # this is copied straight from the lab

stripped_contents = contents.translate(translator)

stripped_contents = stripped_contents.lower()

contents_list = stripped_contents.split()
...
for word in contents_list:
    while word in STOPWORDS and word in contents_list:
        contents_list.remove(word)
    word_count = contents_list.count(word)
    words_dict[word] = word_count

words = list(words_dict.items())
words.sort(key=lambda tup: tup[1], reverse=True) # this is also copied straight from the lab
